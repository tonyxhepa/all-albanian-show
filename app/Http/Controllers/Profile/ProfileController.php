...
class ProfileController extends Controller
{
    ...
    public function create($slug, Menu $menu)
    {
        $user = User::where('slug', $slug)->first();
        $countries = Country::pluck('name', 'id')->all();
        $gjinia = Gjinia::pluck('name', 'id')->all();
        return view('profile.create', compact('countries', 'gjinia', 'user', 'menu'));
    }
    ...
    public function show($slug, $id, Menu $menu)
    {
        $user= User::where('slug', $slug)->first();
        $profile = $user->profile->whereId($id)->first();
        return view('profile.show',[$slug, $id], compact('profile', 'menu'));
        
    }

    public function edit($slug, $id, Request $request, Menu $menu)
    {
        $countries = Country::pluck('name', 'id')->all();
        $gjinia = Gjinia::pluck('name', 'id')->all();
        $user = User::where('slug', $slug)->first();
        ...
        return view('profile.edit',[$slug, $id], compact('profile', 'gjinia', 'countries', 'menu'));
