...
@section('content')
    ...
    @if(count($post->photos) <= 5)
        <div class="form-group">
            <form id="addPhotosForm" action="{{ action('Admin\ShitjeController@addPhoto', $post->id) }}"
                  class="dropzone"
    ...
    <form method="POST" action="{{ route('shitje.update', $post->id) }}" enctype="multipart/form-data">
        {!! csrf_field() !!}
