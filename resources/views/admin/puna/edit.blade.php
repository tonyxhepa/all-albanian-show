...
@section('content')
    ...
    <form method="POST" action="{{ route('puna.update', $post->id) }}" enctype="multipart/form-data">
        ...
        <div id="app">
            <div class="form-group">
                <label for="profesioni_id">Profesioni</label>
                <select name="profesioni_id" id="profesioni_id" class="form-control" v-model="car_make_id" v-on:change="getCarModel()" v-bind:disabled="disableWhenSelect">
                    <option value="{{ old('profesioni_id', $post->profesoni->id) }}">{{ old('profesioni_id', $post->profesioni->name) }}</option>
                    @foreach($profesionet as $item => $value)
        ...
        <div class="form-group">
            <label for="orari_id">Oraret</label>
