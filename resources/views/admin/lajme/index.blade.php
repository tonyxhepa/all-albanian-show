        ...
        @if($lajmet)
            ...
            @foreach($lajmet as $post)
                ...
                <tr>
                    ...
                    <td><a href="{{route('lajme.edit', $post->id)}}">{{ $post->user->name }}</a></td>
                    <td>{{ $post->lajme_cats ? $post->lajme_cats->name : 'Uncategorized' }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ str_limit($post->pershkrimi, 20) }}</td>
                    <td>{{ $post->created_at->diffForHumans() }}</td>
                    <td>{{ $post->updated_at->diffForHumans() }}</td>
                    <td><a href="{{ url('/admin/lajme/'. $post->id .'/edit') }}"><button class="btn btn-primary">edit</button></a> </td>
                    <td>{!! Form::open(['method'=>'DELETE', 'action'=>['Admin\LajmeController@destroy', $post->id]]) !!}
