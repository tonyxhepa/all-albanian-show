        ...
        @if($sport)
            ...
            @foreach($sport as $post)
                ...
                <tr>
                    ...
                    <td><a href="{{route('sport.edit', $post->id)}}">{{ $post->user->name }}</a></td>
                    <td>{{ $post->sport_cats ? $post->sport_cats->name : 'Uncategorized' }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ str_limit($post->body, 20) }}</td>
                    <td>{{ $post->created_at->diffForHumans() }}</td>
                    <td>{{ $post->updated_at->diffForHumans() }}</td>
                    <td><a href="{{ url('/admin/sport/'. $post->id .'/edit') }}"><button class="btn btn-primary">edit</button></a> </td>
                    <td>{!! Form::open(['method'=>'DELETE', 'action'=>['Admin\SportController@destroy', $post->id]]) !!}
