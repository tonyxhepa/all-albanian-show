        ...
        @if($magazinat)
            ...
            @foreach($magazinat as $post)
                ...
                <tr>
                    ...
                    <td><a href="{{route('magazina.edit', $post->id)}}">{{ $post->user->name }}</a></td>
                    <td>{{ $post->magazina_cats ? $post->magazina_cats->name : 'Uncategorized' }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ str_limit($post->pershkrimi, 20) }}</td>
                    <td>{{ $post->created_at->diffForHumans() }}</td>
                    <td>{{ $post->updated_at->diffForHumans() }}</td>
                    <td><a href="{{ url('/admin/magazina/'. $post->id .'/edit') }}"><button class="btn btn-primary">edit</button></a> </td>
                    <td>{!! Form::open(['method'=>'DELETE', 'action'=>['Admin\MagazinaController@destroy', $post->id]]) !!}
