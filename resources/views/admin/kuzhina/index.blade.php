        ...
        @if($kuzhina)
            ...
            @foreach($kuzhina as $post)
                ...
                <tr>
                    ...
                    <td><a href="{{route('kuzhina.edit', $post->id)}}">{{ $post->user->name }}</a></td>
                    <td>{{ $post->kushina_cats ? $post->kushina_cats->name : 'Uncategorized' }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ str_limit($post->pershkrimi, 20) }}</td>
                    <td>{{ $post->created_at->diffForHumans() }}</td>
                    <td>{{ $post->updated_at->diffForHumans() }}</td>
                    <td><a href="{{ url('/admin/kuzhina/'. $post->id .'/edit') }}"><button class="btn btn-primary">edit</button></a> </td>
                    <td>{!! Form::open(['method'=>'DELETE', 'action'=>['Admin\KuzhinaController@destroy', $post->id]]) !!}
