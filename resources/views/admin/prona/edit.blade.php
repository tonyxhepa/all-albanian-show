...
@section('content')
    ...
    @if(count($post->photos) <= 5)
        <div class="form-group">
            <form id="addPhotosForm" action="{{ action('Admin\PronaController@addPhoto', $post->id) }}"
                  class="dropzone"
    ...
    <form method="POST" action="{{ route('prona.update', $post->id) }}" enctype="multipart/form-data">
        {!! csrf_field() !!}
