        ...
        @if($pronat)
            ...
            @foreach($pronat as $post)
                ...
                <tr>
                    ...
                    <td><a href="{{route('prona.edit', $post->id)}}">{{ $post->user->name }}</a></td>
                    <td>{{ $post->prona_cats ? $post->prona_cats->name : 'Uncategorized' }}</td>
                    <td>{{ $post->title }}</td>
                    <td>{{ str_limit($post->body, 20) }}</td>
                    <td>{{ $post->created_at->diffForHumans() }}</td>
                    <td>{{ $post->updated_at->diffForHumans() }}</td>
                    <td><a href="{{ url('/admin/prona/'. $post->id .'/edit') }}"><button class="btn btn-primary">edit</button></a> </td>
                    <td>{!! Form::open(['method'=>'DELETE', 'action'=>['Admin\PronaController@destroy', $post->id]]) !!}
