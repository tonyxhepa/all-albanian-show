...
@section('content')
    @foreach($kategory as $cat)
        @if($cat->slug == 'video')
            <div class="container">
                <div class="row" style="background-color: #8eb4cb;"><h3><a href="{{ url('femrat/kategory/'. $cat->slug) }}">{{ $cat->name }}</a> </h3></div>
                <div class="row" style="background-color: #8eb4cb;">
                    @foreach($cat->femrat()->latest() as $four)
                        <div class="col-sm-3">
        ...
        @elseif($cat->slug == 'premier-liga')
            <div class="container">
                <div class="row" style="background-color: #8a6d3b;"><h3><a href="{{ url('fenrat/kategory/'. $cat->slug) }}">{{ $cat->name }}</a> </h3></div>
        ...
        @else
            <div class="container">
                <div class="row"><h3><a href="{{ url('femrat/kategory/'. $cat->slug) }}">{{ $cat->name }}</a> </h3></div>


                <div class="row">
                    @foreach($cat->femrat as $four)
                        <div class="col-sm-3">
                            @if(count($four->photos) >= 1)
                                <img class="img-responsive" style="height: 90px;"  src="{{ asset('storage').$four->photos->first()->thumbnail }}" alt="">
                            @endif
